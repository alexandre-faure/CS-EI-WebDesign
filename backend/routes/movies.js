...
async function getFilmsPopulaires(dates, genres, searchBar) {
    try {
      const response = await getMovies(`https://api.themoviedb.org/3/movie/popular?api_key=522d421671cf75c2cba341597d86403a`, dates, genres, searchBar, nbMovies, nbPages);
      return response;
    } catch (error) {
      console.error(`Erreur lors de la récupération des détails des films populaires`);
      return null;
