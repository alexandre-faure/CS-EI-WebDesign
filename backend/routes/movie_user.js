...
router.post('', function (req, res) {
    ...
    .then((movie_user) => {
        ...
        if (movie_user.length == 1){
            ...
            if ('status' in params){
                value_user_movie = {...value_user_movie,
            ...
            appDataSource
                ...
                .catch(function () {
                    res.status(500).json({ message: 'Une erreur est survenue lors de la mise à jour du movie_user.' });
                });
        }
        else{
            // On crée la valeur
            appDataSource
                ...
                .catch(function () {
                    res.status(500).json({ message: 'Une erreur est survenue lors de la création du movie_user.' });
                });
